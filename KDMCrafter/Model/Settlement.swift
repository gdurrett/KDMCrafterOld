...
class Settlement {
    ...
    init(name: String) {
        ...
        let scavengerKit = Gear(name: "Scavenger Kit", description: GearDescription(type: .item, stats: .item("N/A"), affinities: [.greenDown], detailText: NSMutableAttributedString().bold("Unique").normal("\n\nWhen you defeat a monster, gain either 1 random basic resource or 1 random monster resource from that monster's resource deck.")), qtyAvailable: 1,  resourceTypeRequirements: [.scrap:1], resourceSpecialRequirements: [.pelt:1], locationRequirement: barberSurgeon)
        ...
        let boneClub = Gear(name: "Bone Club", description: GearDescription(type: .weapon, stats: .weapon(2, 6, 5), affinities: [.redLeft, .redRight], detailText: NSMutableAttributedString().boldCenter("Cumbersome").normal(": Spend a movement as an additional cost to activate this weapon. Ignore Cumbersome if this weapon is activated indirectly (Pounce, Charge, etc).")), qtyAvailable: 3, resourceTypeRequirements: [.bone:3], locationRequirement: boneSmith)
